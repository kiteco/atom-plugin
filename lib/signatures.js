...
module.exports = class KiteSignature {
  init(kite) {
    ...
    this.editors = Kite.getModule('editors');
    ...
    this.subscriptions.add(atom.workspace.onDidChangeActiveTextEditor(editor => {
      ...
      if (editor && this.editors && this.editors.isGrammarSupported(editor)) {
  ...
  unsubscribeFromActiveEditor() {
    if (this.activeEditor) {
      this.subscriptions && this.subscriptions.remove(this.activeEditorSubscriptions);
  ...
  requestSignatureCheck() {
    ...
    request(() => {
      if (!this.editors.isEditorWhitelisted(this.activeEditor)) {
  ...
  setListElement(listElement) {
    ...
    const intersectionObserver = new IntersectionObserver(([entry]) => {
      ...
      if (this.listElement.parentNode && !mutationObserverDisposable) {
        ...
        this.subscriptions && this.subscriptions.remove(mutationObserverDisposable);
      ...
      if (data) {
        if (!this.signature) {
          ...
          this.signature.onDidDetach = () => {
            this.subscriptions && this.subscriptions.remove(this.scrollDisposable);
  ...
  requestOverflowCheck() {
    ...
    requestAnimationFrame(() => {
      ...
      // It's not guaranted that we'll get a view while running tests
      if (!view) { return; }
      ...
      const shouldFollowCompletion = this.completionVisible;
  ...
  stickToCompletions() {
    ...
    if (sigBounds.width <= spaceRight) {
      ...
      this.signature.style.width = `${compBounds.width}px`;
  ...
  checkCompletionOverlap() {
    if (this.decoration && this.completionVisible) {
      const sigBounds = this.signature.getBoundingClientRect();
