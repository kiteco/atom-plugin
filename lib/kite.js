...
const Kite = (module.exports = {
  ...
  activate() {
    ...
    const toRemoveConfigs = [
      'signatureKwargsVisible',
      'signaturePopularPatternsVisible',
    ];
    const kiteCfg = atom.config.getAll('kite');
    if (kiteCfg && kiteCfg.length > 0) {
      const configs = Object.keys(kiteCfg[0].value);
      configs.forEach(config => {
        if (toRemoveConfigs.includes(config)) {
          atom.config.unset(`kite.${config}`);
        }
      });
    }
