...
const Kite = (module.exports = {
  ...
  registerModule(name, mod) {
    ...
    if (this.modules && name in this.modules) {
      throw new Error(
        `a module with name '${name}' has already been registered`
      );
    }
  ...
  activate() {
    ...
    this.subscriptions.add(
      atom.workspace.onDidOpen(e => {
        if (e.uri === 'atom://config' && !this.settingsViewSubscription) {
          ...
          this.settingsViewSubscription = new DisposableEvent(
            ...
            () => {
              setTimeout(() => {
                ...
                if (
                  kiteSettings &&
                  kiteSettings.element.style.display !== 'none'
                ) {
    ...
    this.subscriptions.add(
      this.app.onDidInstall(() => this.connect('onDidInstall'))
    );
    this.subscriptions.add(
      this.app.onDidStart(() => this.connect('onDidStart'))
    );
    ...
    const settingsWarningNotif = using => {
      if (using) {
        const notif = atom.notifications.addWarning(
          'Kite is not configured properly',
          {
            description:
              'You must turn off Atom\'s tree-sitter parser and autoconfirm single suggestion settings for the best experience. <a href="https://help.kite.com/article/80-why-do-i-need-to-turn-off-tree-sitter">Learn more</a>\n\nRestart will occur after a few seconds.',
            dismissable: true,
            buttons: [
              {
    ...
    this.subscriptions.add(
      atom.config.observe('core.useTreeSitterParsers', settingsWarningNotif),
      atom.config.observe(
        'autocomplete-plus.enableAutoConfirmSingleSuggestion',
        settingsWarningNotif
      )
    );
      ...
      atom.commands.add('atom-workspace', {
        ...
        'kite:package-settings': () =>
          atom.applicationDelegate.openExternal(
            'atom://settings-view/show-package?package=kite'
          ),
        'kite:help': () =>
          atom.applicationDelegate.openExternal(
            'https://help.kite.com/category/43-atom-integration'
          ),
        'kite:status': () => {
          metrics.featureRequested('status_panel');
          this.getStatusPanel()
    ...
    this.app.connect('activation').then(state => {
      if (
        state === KiteApp.STATES.UNINSTALLED &&
        ...
        if (
          state === KiteApp.STATES.NOT_RUNNING &&
          atom.config.get('kite.startKiteAtStartup')
        ) {
  ...
  getEditorsForPath(path) {
    return atom.workspace
      .getPaneItems()
      .filter(i => i && i.getURI && i.getURI() === path);
  },
      ...
      .then(autocompletePlus => {
        ...
        if (atom.config.get('kite.enableSnippets')) {
          atom.packages.activatePackage('snippets').then(snippetsPkg => {
            ...
            const safeGoToPreviousTabStop =
              snippets && snippets.goToPreviousTabStop;
        ...
        const safeShowOrHide =
          manager && manager.showOrHideSuggestionListForBufferChanges;
        const safeConfirm = manager && manager.confirm;
        const safeReplaceTextWithMatch =
          manager && manager.replaceTextWithMatch;
        ...
        if (safeDisplay) {
          ...
          manager.confirm = suggestion => {
            confirmed = true;
            wasSnippetConfirmed =
              suggestion.className === 'kite-completion' &&
              typeof suggestion.snippet !== 'undefined';
            ...
            if (atom.config.get('kite.enableSnippets')) {
              ...
              const apiVersion = manager.providerManager.apiVersionForProvider(
                suggestion.provider
              );
              const triggerPosition = manager.editor
                .getLastCursor()
                .getBufferPosition();
            ...
            if (
              ...
              editor.transact(() => {
                ...
                if (suggestion.snippet && manager.snippetsManager) {
                  manager.snippetsManager.insertSnippet(
                    suggestion.snippet,
                    editor,
                    cursor
                  );
                  ...
                  cursor.selection.insertText(
                    suggestion.text ? suggestion.text : suggestion.snippet,
                    {
          ...
          manager.hideSuggestionList = () => {
            ...
            if (typeof editor !== 'undefined') {
              ...
              if (
                ...
                element.querySelector('.suggestion-description').style.display =
                  'none';
              } else {
          ...
          manager.buffer &&
            manager.buffer.onDidChangeText(
              manager.showOrHideSuggestionListForBufferChanges
            );
          ...
          manager.displaySuggestions = (suggestions, options) => {
            if (element.querySelector('kite-signature')) {
              ...
              if (parseFloat(autocompletePlus.metadata.version) >= 2.36) {
                manager.showSuggestionList(
                  manager.getUniqueSuggestions(suggestions, s => s.text),
                  options
                );
              } else {
                manager.showSuggestionList(
                  manager.getUniqueSuggestions(suggestions),
                  options
                );
              }
            } else {
        ...
        if (safeUpdate) {
          listElement.updateDescription = item => {
            ...
            if (item && item.descriptionHTML) {
              listElement.descriptionContainer.classList.add(
                'kite-completions'
              );
