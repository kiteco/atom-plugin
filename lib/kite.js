  ...
  provideDefinitions() {
    return {
      ...
      getDefinition: (editor, position) => {
        ...
        const token = kiteEditor.tokensList.tokenAtPosition(position);

        if (token) {
          const range = kiteEditor.tokensList.tokenRange(token);

          return DataLoader.getHoverDataAtRange(editor, range)
          .then(data => {
  ...
  consumeDatatipService(datatipService) {
    datatipService.addProvider({
      ...
      datatip: (editor, bufferPosition) => {
        const kiteEditor = this.kiteEditorForEditor(editor);
        const token = kiteEditor.tokensList.tokenAtPosition(bufferPosition);

        if (token) {
          const range = Range.fromObject([
            editor.getBuffer().positionForCharacterIndex(token.begin_bytes),
            editor.getBuffer().positionForCharacterIndex(token.end_bytes),
          ]);

          return DataLoader.getHoverDataAtRange(editor, range)
          .then(data => {
            if (data && data.symbol && data.symbol.length) {
              const {internalExpandRangeURL, internalOpenRangeInWebURL, internalGotoURL} = require('./urls');
              const {symbolId, symbolKind, symbolName, idIsEmpty} = require('./kite-data-utils');
              const [symbol] = data.symbol;
              // const type = symbolType(symbol);
              const actions = [
                `[more](${internalExpandRangeURL(range)})`,
              ];

              if (!idIsEmpty(symbolId(symbol))) {
                actions.unshift(`[web](${internalOpenRangeInWebURL(range)})`);
              }
