...
let child_process, CompositeDisposable, TextEditor, Range, Point,
  KiteAPI, Logger, completions, metrics, KiteApp,
  KiteStatus, KiteStatusPanel,  NotificationsCenter,
  RollbarReporter, OverlayManager, KiteEditor, DataLoader,
  DisposableEvent, EditorEvents, AccountManager, VirtualCursor;
...
const Kite = module.exports = {
  activate() {
    ...
    if (!AccountManager) {
      ...
      ({CompositeDisposable, TextEditor, Range, Point} = require('atom'));
  ...
  provideDefinitions() {
    return {
      ...
      getDefinition: (editor, position) => {
        return DataLoader.getHoverDataAtPosition(editor, position)
        .then(data => {
          const def = data && data.report && data.report.definition;
          return def
          ? {
            queryRange: Range.fromObject([position, position]),
            definitions: [{
              language: data.language,
              position: Point.fromObject([
                def.line - 1,
                0,
              ]),
              path: def.filename,
  ...
  consumeDatatipService(datatipService) {
    datatipService.addProvider({
      ...
      datatip: (editor, bufferPosition) => {
        return DataLoader.getHoverDataAtPosition(editor, bufferPosition)
        .then(data => {
          if (data && data.symbol && data.symbol.length) {
            const {internalExpandPositionURL, internalOpenPositionInWebURL, internalGotoURL} = require('./urls');
            const {symbolId, symbolKind, symbolName, idIsEmpty} = require('./kite-data-utils');
            const [symbol] = data.symbol;
            // const type = symbolType(symbol);
            const actions = [
              `[more](${internalExpandPositionURL(bufferPosition)})`,
            ];

            if (!idIsEmpty(symbolId(symbol))) {
              actions.unshift(`[web](${internalOpenPositionInWebURL(bufferPosition)})`);
            }

            if (data.report.definition &&
                data.report.definition.filename &&
                data.report.definition.filename.trim() !== '') {
              actions.unshift(`[def](${internalGotoURL(data.report.definition)})`);
            }
            ...
            return {
              ...
              range: Range.fromObject([bufferPosition, bufferPosition]),
              pinnable: true,
            };
          }
        })
  ...
  consumeSignatureHelp(register) {
    const {stripTags, getFunctionDetails, promisifyReadResponse} = require('./utils');
    const {valueLabel, parameterType} = require('./kite-data-utils');
    ...
    register({
      ...
      getSignatureHelp(editor, point) {
        return DataLoader.getSignaturesAtPosition(editor, point)
        .then(data => {
          console.log(data);
          if (data) {
