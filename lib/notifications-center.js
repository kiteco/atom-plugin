...
class NotificationsCenter {
  ...
  constructor(kiteApp) {
    ...
    this.subscriptions.add(
      ...
      this.app.onKiteInstallUnavailable(() => {
      ...
      this.app.onKiteInstallAvailable(installFn => {
        KiteAPI.checkHealth().then(state => {
          if (state !== KiteAPI.STATES.UNINSTALLED) {
            return;
          }
          if (atom.config.get('kite.installPausedNotifShown')) {
            ...
            this.showInstallUnpausedNotif(installFn); // TODO: pass Install Func?
            return;
          }
      ...
      'Temporarily Unable to Install',
      {
        ...
        buttons: [
          ...
          },
          {
            text: 'Learn more',
            onDidClick(dismiss) {
              atom.applicationDelegate.openExternal('https://kite.com/kite-is-temporarily-unavailable/?source=vscode');
              dismiss && dismiss();
            },
          },
        ],
