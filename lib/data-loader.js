...
const DataLoader = {
  ...
  getSnippetCompletionsAtPosition(editor, position) {
    ...
    const text = buffer.getText();
    const cursorPosition = buffer.characterIndexForPosition(position);
    return KiteAPI.getSnippetCompletionsAtPosition(editor.getPath(), text, cursorPosition, 'atom')
    .then(completions => {
      return completions.map(c => {
        const completion = {
          text: c.snippet.text,
        ...
        if (c.snippet.placeholders.length > 0) {
          ...
          var offset = 0;
          for (var i = 0; i < c.snippet.placeholders.length; i++) {
          ...
          // Strip () from being inserted into buffer
          completion.snippet = snippetStr.slice(1, snippetStr.length - 1);
        }
        completion.type = c.hint;
        completion.rightLabelHTML = kindLabel(c.hint);
        if (c.documentation) {
          completion.descriptionHTML = appendCompletionsFooter(c, escapeHTML(c.documentation));
          completion.description = c.documentation;
        }
        return completion;
      });
    });
